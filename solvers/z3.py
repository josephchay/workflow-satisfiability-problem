...
from z3 import Solver, Bool, If, Not, And, Or, Implies, is_true, sat
...
class Z3UDPBWSPSolver(BaseWSPSolver):
    ...
    def solve(self) -> Dict:
        ...
        for s in range(self.instance.number_of_steps):
            solver.add(sum([If(x[s][u], 1, 0) for u in range(self.instance.number_of_users)]) == 1)
        ...
        if self.active_constraints['at_most_k']:
            for k, steps in self.instance.at_most_k:
                ...
                solver.add(sum([If(z[u], 1, 0) for u in range(self.instance.number_of_users)]) <= k)
        ...
        if self.active_constraints['one_team']:
            for steps, teams in self.instance.one_team:
                ...
                solver.add(sum([If(t, 1, 0) for t in team_selected]) == 1)
...
class Z3PBPBWSPSolver(BaseWSPSolver):
    ...
    def solve(self) -> Dict:
        ...
        for s1, s2 in itertools.combinations(range(self.instance.number_of_steps), 2):
            for u in range(self.instance.number_of_users):
                solver.add(Implies(M[s1][s2], x[s1][u] == x[s2][u]))
        ...
        while len(solutions) < self.solution_limit and solver.check() == sat:
            solution_count += 1
            model = solver.model()
            ...
            for s in range(self.instance.number_of_steps):
                for u in range(self.instance.number_of_users):
                    if is_true(model.evaluate(x[s][u])):
                        solution.append({'step': s + 1, 'user': u + 1})
