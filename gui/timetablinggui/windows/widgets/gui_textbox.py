...
class GUITextbox(GUIBaseClass):
    ...
    def __init__(self,
        ...
        # transfer basic functionality (_bg_color, size, __appearance_mode, scaling) to GUIBaseClass
        super().__init__(master=master, bg_color=bg_color, width=width, height=height)

        # color
        self._fg_color = ThemeManager.theme["GUITextbox"]["fg_color"] if fg_color is None else self._check_color_type(fg_color, transparency=True)
        self._border_color = ThemeManager.theme["GUITextbox"]["border_color"] if border_color is None else self._check_color_type(border_color)
        self._text_color = ThemeManager.theme["GUITextbox"]["text_color"] if text_color is None else self._check_color_type(text_color)
        self._scrollbar_button_color = ThemeManager.theme["GUITextbox"]["scrollbar_button_color"] if scrollbar_button_color is None else self._check_color_type(scrollbar_button_color)
        self._scrollbar_button_hover_color = ThemeManager.theme["GUITextbox"]["scrollbar_button_hover_color"] if scrollbar_button_hover_color is None else self._check_color_type(scrollbar_button_hover_color)

        # shape
        self._corner_radius = ThemeManager.theme["GUITextbox"]["corner_radius"] if corner_radius is None else corner_radius
        self._border_width = ThemeManager.theme["GUITextbox"]["border_width"] if border_width is None else border_width
        self._border_spacing = border_spacing

        # font
        self._font = GUIFont() if font is None else self._check_font_type(font)
        if isinstance(self._font, GUIFont):
            self._font.add_size_configure_callback(self._update_font)

        self._canvas = GUICanvas(master=self,
        ...
        self._y_scrollbar = GUIScrollbar(self,
                                         width=8,
                                         height=0,
                                         border_spacing=0,
        ...
        self._x_scrollbar = GUIScrollbar(self,
                                         height=8,
                                         width=0,
                                         border_spacing=0,
                                         fg_color=self._fg_color,
    ...
    def configure(self, require_redraw=False, **kwargs):
        if "fg_color" in kwargs:
            ...
            # check if GUI widgets are children of the frame and change their _bg_color to new frame fg_color
            for child in self.winfo_children():
                if isinstance(child, GUIBaseClass) and hasattr(child, "_fg_color"):
