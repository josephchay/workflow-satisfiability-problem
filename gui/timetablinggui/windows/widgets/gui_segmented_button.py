...
class GUISegmentedButton(GUIFrame):
    ...
    def __init__(self,
        ...
        self._sb_fg_color = ThemeManager.theme["GUISegmentedButton"]["fg_color"] if fg_color is None else self._check_color_type(fg_color)

        self._sb_selected_color = ThemeManager.theme["GUISegmentedButton"]["selected_color"] if selected_color is None else self._check_color_type(selected_color)
        self._sb_selected_hover_color = ThemeManager.theme["GUISegmentedButton"]["selected_hover_color"] if selected_hover_color is None else self._check_color_type(selected_hover_color)

        self._sb_unselected_color = ThemeManager.theme["GUISegmentedButton"]["unselected_color"] if unselected_color is None else self._check_color_type(unselected_color)
        self._sb_unselected_hover_color = ThemeManager.theme["GUISegmentedButton"]["unselected_hover_color"] if unselected_hover_color is None else self._check_color_type(unselected_hover_color)

        self._sb_text_color = ThemeManager.theme["GUISegmentedButton"]["text_color"] if text_color is None else self._check_color_type(text_color)
        self._sb_text_color_disabled = ThemeManager.theme["GUISegmentedButton"]["text_color_disabled"] if text_color_disabled is None else self._check_color_type(text_color_disabled)

        self._sb_corner_radius = ThemeManager.theme["GUISegmentedButton"]["corner_radius"] if corner_radius is None else corner_radius
        self._sb_border_width = ThemeManager.theme["GUISegmentedButton"]["border_width"] if border_width is None else border_width
        ...
        self._font = GUIFont() if font is None else font
        self._state = state

        self._buttons_dict: Dict[str, GUIButton] = {}  # mapped from value to button object
        if values is None:
            self._value_list: List[str] = ["GUISegmentedButton"]
        else:
    ...
    def _get_index_by_value(self, value: str):
        ...
        raise ValueError(f"GUISegmentedButton does not contain value '{value}'")
    ...
    def _create_button(self, index: int, value: str) -> GUIButton:
        new_button = GUIButton(self,
                               width=0,
    ...
    def _check_unique_values(values: List[str]):
        ...
        if len(values) != len(set(values)):
            raise ValueError("GUISegmentedButton values are not unique")
    ...
    def insert(self, index: int, value: str):
        if value not in self._buttons_dict:
            if value != "":
                ...
                raise ValueError(f"GUISegmentedButton can not insert value ''")
        else:
            raise ValueError(f"GUISegmentedButton can not insert value '{value}', already part of the values")
    ...
    def move(self, new_index: int, value: str):
        if 0 <= new_index < len(self._value_list):
            if value in self._buttons_dict:
                ...
                raise ValueError(f"GUISegmentedButton has no value named '{value}'")
        else:
            raise ValueError(f"GUISegmentedButton new_index {new_index} not in range of value list with len {len(self._value_list)}")
