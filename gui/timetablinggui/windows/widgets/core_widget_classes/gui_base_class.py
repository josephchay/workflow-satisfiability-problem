...
from ..image import GUIImage
from ..appearance_mode import GUIAppearanceModeBaseClass
from ..scaling import GUIScalingBaseClass

from ..utility import pop_from_dict_by_set, check_kwargs_empty


class GUIBaseClass(tkinter.Frame, GUIAppearanceModeBaseClass, GUIScalingBaseClass):
    """ Base class of every GUI widget, handles the dimensions, bg_color,
        appearance_mode changes, scaling, bg changes of master if master is not a GUI widget """
    ...
    def __init__(self,
        ...
        GUIAppearanceModeBaseClass.__init__(self)
        GUIScalingBaseClass.__init__(self, scaling_type="widget")
        ...
        # overwrite configure methods of master when master is tkinter widget, so that bg changes get applied on child GUI widget as well
        if isinstance(self.master, (tkinter.Tk, tkinter.Toplevel, tkinter.Frame, tkinter.LabelFrame, ttk.Frame, ttk.LabelFrame, ttk.Notebook)) and not isinstance(self.master, (GUIBaseClass, GUIAppearanceModeBaseClass)):
    ...
    def destroy(self):
        ...
        GUIAppearanceModeBaseClass.destroy(self)
        GUIScalingBaseClass.destroy(self)
    ...
    def config(self, *args, **kwargs):
        raise AttributeError("'config' is not implemented for GUI widgets. For consistency, always use 'configure' instead.")
    ...
    def _check_font_type(self, font: any):
        ...
        if isinstance(font, GUIFont):
            ...
            raise ValueError(f"Wrong font type {type(font)}\n" +
                             f"For consistency, timetablinggui requires the font argument to be a tuple of len 2 to 6 or an instance of GUIFont.\n" +
                             f"\nUsage example:\n" +
                             f"font=timetablinggui.GUIFont(family='<name>', size=<size in px>)\n" +
    ...
    def _check_image_type(self, image: any):
        ...
        if image is None:
            return image
        elif isinstance(image, GUIImage):
            return image
        else:
            warnings.warn(f"{type(self).__name__} Warning: Given image is not GUIImage but {type(image)}. Image can not be scaled on HighDPI displays, use GUIImage instead.\n")
            return image
    ...
    def _detect_color_of_master(self, master_widget=None) -> Union[str, Tuple[str, str]]:
        ...
        if isinstance(master_widget, (windows.widgets.core_widget_classes.GUIBaseClass, windows.TimetablingGUI, windows.GUIToplevel, windows.widgets.gui_scrollable_frame.GUIScrollableFrame)):
            if master_widget.cget("fg_color") is not None and master_widget.cget("fg_color") != "transparent":
                return master_widget.cget("fg_color")

            elif isinstance(master_widget, windows.widgets.gui_scrollable_frame.GUIScrollableFrame):
