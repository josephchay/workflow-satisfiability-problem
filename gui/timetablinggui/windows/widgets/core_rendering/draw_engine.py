...
class DrawEngine:
    """
    This is the core of the timetablinggui library where all the drawing on the tkinter.Canvas happens.
    ...
    def __init__(self, canvas: GUICanvas):
        self._canvas = canvas
    ...
    def draw_checkmark(self, width: Union[float, int], height: Union[float, int], size: Union[int, float]) -> bool:
        ...
        if self.preferred_drawing_method == "polygon_shapes" or self.preferred_drawing_method == "circle_shapes":
            ...
            if not self._canvas.find_withtag("checkmark"):
                self._canvas.create_text(0, 0, text="Z", font=("timetablinggui_shapes_font", -size), tags=("checkmark", "create_text"), anchor=tkinter.CENTER)
                self._canvas.tag_raise("checkmark")
                requires_recoloring = True
    ...
    def draw_dropdown_arrow(self, x_position: Union[int, float], y_position: Union[int, float], size: Union[int, float]) -> bool:
        ...
        if self.preferred_drawing_method == "polygon_shapes" or self.preferred_drawing_method == "circle_shapes":
            ...
            if not self._canvas.find_withtag("dropdown_arrow"):
                self._canvas.create_text(0, 0, text="Y", font=("timetablinggui_shapes_font", -size), tags="dropdown_arrow", anchor=tkinter.CENTER)
                self._canvas.tag_raise("dropdown_arrow")
                requires_recoloring = True

            self._canvas.itemconfigure("dropdown_arrow", font=("timetablinggui_shapes_font", -size))
