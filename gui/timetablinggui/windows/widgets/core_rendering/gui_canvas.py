...
class GUICanvas(tkinter.Canvas):
    ...
    to look best for rendering timetablinggui shapes on the different operating systems.
    ...
    font 'timetablinggui_shapes_font'. It contains circle shapes with different sizes filling
    ...
    def create_aa_circle(self, x_pos: int, y_pos: int, radius: int, angle: int = 0, fill: str = "white",
        ...
        circle_1 = self.create_text(x_pos, y_pos, text=self._get_char_from_radius(radius), anchor=anchor, fill=fill,
                                    font=("timetablinggui_shapes_font", -radius * 2), tags=tags, angle=angle)
        self.addtag_withtag("gui_aa_circle_font_element", circle_1)
    ...
    def coords(self, tag_or_id, *args):
        ...
        if type(tag_or_id) == str and "gui_aa_circle_font_element" in self.gettags(tag_or_id):
            coords_id = self.find_withtag(tag_or_id)[0]  # take the lowest id for the given tag
            super().coords(coords_id, *args[:2])

            if len(args) == 3:
                super().itemconfigure(coords_id, font=("timetablinggui_shapes_font", -int(args[2]) * 2), text=self._get_char_from_radius(args[2]))

        elif type(tag_or_id) == int and tag_or_id in self._aa_circle_canvas_ids:
            super().coords(tag_or_id, *args[:2])

            if len(args) == 3:
                super().itemconfigure(tag_or_id, font=("timetablinggui_shapes_font", -args[2] * 2), text=self._get_char_from_radius(args[2]))
