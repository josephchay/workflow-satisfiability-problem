from ...widgets.gui_frame import GUIFrame
from ...widgets.core_rendering import GUICanvas
from ...widgets.font import GUIFont
from ...widgets.theme import ThemeManager
from ...widgets.gui_button import GUIButton
from ...widgets.gui_entry import GUIEntry
from ...widgets.gui_label import GUILabel
from tkinter import END
import copy


class TableManager(GUIFrame):
    """ table Widget """
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.hover_color = ThemeManager.theme["GUIButton"][
            "hover_color"] if hover_color is None else hover_color
        self.orient = orientation
        self.border_color = ThemeManager.theme["GUIButton"][
            "border_color"] if border_color is None else border_color
        self.inside_frame = GUIFrame(self, border_width=0, fg_color="transparent")
        ...
        self.text_color = ThemeManager.theme["GUILabel"]["text_color"] if text_color is None else text_color
        self.font = font
        # if colors are None then use the default frame colors:
        self.data = {}
        self.fg_color = ThemeManager.theme["GUIFrame"]["fg_color"] if not self.colors[0] else \
        self.colors[0]
        self.fg_color2 = ThemeManager.theme["GUIFrame"]["top_fg_color"] if not self.colors[1] else \
        self.colors[1]

        if self.colors[0] is None and self.colors[1] is None:
            if self.fg_color == self.master.cget("fg_color"):
                self.fg_color = ThemeManager.theme["GUI"]["fg_color"]
            if self.fg_color2 == self.master.cget("fg_color"):
                self.fg_color2 = ThemeManager.theme["GUI"]["fg_color"]
        ...
        for i in range(self.rows):
            for j in range(self.columns):
                ...
                if self.write:
                    ...
                    self.frame[i, j] = GUIEntry(self.inside_frame,
                                                               font=self.font,
                                                               corner_radius=0,
                                                               **args)
                    if value is None:
                    ...
                    if value is None:
                        value = " "
                    self.frame[i, j] = GUIButton(self.inside_frame, background_corner_colors=corners,
                                                                font=self.font,
