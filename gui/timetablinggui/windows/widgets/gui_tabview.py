...
class GUITabview(GUIBaseClass):
    ...
    def __init__(self,
        ...
        # transfer some functionality to GUIFrame
        super().__init__(master=master, bg_color=bg_color, width=width, height=height, **kwargs)

        # color
        self._border_color = ThemeManager.theme["GUIFrame"]["border_color"] if border_color is None else self._check_color_type(border_color)

        # determine fg_color of frame
        if fg_color is None:
            if isinstance(self.master, (GUIFrame, GUITabview)):
                if self.master.cget("fg_color") == ThemeManager.theme["GUIFrame"]["fg_color"]:
                    self._fg_color = ThemeManager.theme["GUIFrame"]["top_fg_color"]
                else:
                    self._fg_color = ThemeManager.theme["GUIFrame"]["fg_color"]
            else:
                self._fg_color = ThemeManager.theme["GUIFrame"]["fg_color"]
        else:
        ...
        # shape
        self._corner_radius = ThemeManager.theme["GUIFrame"]["corner_radius"] if corner_radius is None else corner_radius
        self._border_width = ThemeManager.theme["GUIFrame"]["border_width"] if border_width is None else border_width
        self._anchor = anchor
        ...
        self._segmented_button = GUISegmentedButton(self,
                                                    values=[],
        ...
        self._tab_dict: Dict[str, GUIFrame] = {}
    ...
    def winfo_children(self) -> List[any]:
        """
        winfo_children of GUITabview without canvas and segmented button widgets,
        because it's not a child but part of the GUITabview itself
        """
    ...
    def _create_tab(self) -> GUIFrame:
        new_tab = GUIFrame(self,
                           height=0,
                           width=0,
                           border_width=0,
                           corner_radius=0)
    ...
    def tab(self, name: str) -> GUIFrame:
        """ returns reference to the tab with given name """

        if name in self._tab_dict:
            return self._tab_dict[name]
        else:
            raise ValueError(f"GUITabview has no tab named '{name}'")

    def insert(self, index: int, name: str) -> GUIFrame:
        ...
        if name not in self._tab_dict:
            ...
            raise ValueError(f"GUITabview already has tab named '{name}'")

    def add(self, name: str) -> GUIFrame:
    ...
    def move(self, new_index: int, name: str):
        if 0 <= new_index < len(self._name_list):
            if name in self._tab_dict:
                ...
                raise ValueError(f"GUITabview has no name '{name}'")
        else:
            raise ValueError(f"GUITabview new_index {new_index} not in range of name list with len {len(self._name_list)}")
