...
class GUIScalingBaseClass:
    ...
    def _apply_font_scaling(self, font: Union[Tuple, GUIFont]) -> tuple:
        """ Takes GUIFont object and returns tuple font with scaled size, has to be called again for every change of font object """
        assert self.__scaling_type == "widget"
        ...
        elif isinstance(font, GUIFont):
            return font.create_scaled_tuple(self.__widget_scaling)
        else:
            raise ValueError(f"Can not scale font '{font}' of type {type(font)}. font needs to be tuple or instance of GUIFont")
