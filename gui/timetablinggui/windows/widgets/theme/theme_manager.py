...
class ThemeManager:
    ...
    def load_theme(cls, theme_name_or_path: str):
        ...
        if theme_name_or_path in cls._built_in_themes:
            timetablinggui_path = pathlib.Path(script_directory).parent.parent.parent
            with open(os.path.join(timetablinggui_path, "assets", "themes", f"{theme_name_or_path}.json"), "r") as f:
        ...
        if "GUICheckbox" in cls.theme.keys():
            cls.theme["GUICheckBox"] = cls.theme.pop("GUICheckbox")
        if "GUIRadiobutton" in cls.theme.keys():
            cls.theme["GUIRadioButton"] = cls.theme.pop("GUIRadiobutton")
