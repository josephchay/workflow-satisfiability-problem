...
class ScrollableCheckBoxFrame(timetablinggui.GUIScrollableFrame):
    ...
    def add_item(self, item):
        checkbox = timetablinggui.GUICheckBox(self, text=item)
...
class ScrollableRadiobuttonFrame(timetablinggui.GUIScrollableFrame):
    def __init__(self, master, item_list, command=None, **kwargs):
        super().__init__(master, **kwargs)

        self.command = command
        self.radiobutton_variable = timetablinggui.StringVar()
        self.radiobutton_list = []
        for i, item in enumerate(item_list):
            self.add_item(item)

    def add_item(self, item):
        radiobutton = timetablinggui.GUIRadioButton(self, text=item, value=item, variable=self.radiobutton_variable)
        if self.command is not None:
...
class ScrollableLabelButtonFrame(timetablinggui.GUIScrollableFrame):
    ...
    def add_item(self, item, image=None):
        label = timetablinggui.GUILabel(self, text=item, image=image, compound="left", padx=5, anchor="w")
        button = timetablinggui.GUIButton(self, text="Command", width=100, height=24)
        if self.command is not None:
...
class App(timetablinggui.TimetablingGUI):
    def __init__(self):
        ...
        self.title("GUIScrollableFrame example")
        ...
        for i in range(20):  # add items with images
            self.scrollable_label_button_frame.add_item(f"image and item {i}", image=timetablinggui.GUIImage(Image.open(os.path.join(current_dir, "test_images", "chat_light.png"))))
